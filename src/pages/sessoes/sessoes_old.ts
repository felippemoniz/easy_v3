...
tag.selecionado = !tag.selecionado;
   ...
   if (tag.selecionado){
     this.mostraSessaoCom(tag)
   }else{
     this.escondeSessaoCom(tag)
   }
...
var item, tipoSessao
  ...
  for (var i = 0; i < this.sessoes.length; i++) {
      ...
      console.log(tipoSessao + " = " + tag.nomeDetalhado + "-->" + tipoSessao.indexOf(tag.nomeDetalhado))
      if (item.selecionado == 0){
              if (tipoSessao.indexOf(tag.nomeDetalhado)==-1){
                  item.selecionado = 1;
              }
      }
  }
